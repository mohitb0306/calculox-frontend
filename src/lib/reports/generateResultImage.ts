...
export const generateResultImage = (report: ShareableReport, siteInfo?: SiteInfo): Promise<Blob> => {
  return new Promise((resolve, reject) => {
    ...
    if (siteInfo?.name && siteInfo?.url) {
      // Placed on the SAME row/baseline as the date line above (right-aligned),
      // not a new row below it — the card shape drawn earlier clips anything
      // closer than ~32px to the canvas edge, so a lower row would render
      // invisibly. Reusing this proven-safe baseline avoids that.
      ctx.fillStyle = '#b4bcc7';
      ctx.font = `500 16px ${FONT}`;
      ctx.textAlign = 'right';
      ctx.fillText(`Generated by ${siteInfo.name} at ${siteInfo.url}`, W - 72, H - 48);
      ctx.textAlign = 'left';
    }
